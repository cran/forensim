...
# patch 4.1, Delft
LRmixTK <-function(verbose = TRUE)
{
  print_if_verbose <- function(...) if (verbose) print(...)
	...
	#--------- Performance plots ------------------#
      #Simulation subframe:
		...
		sim.loc<-function()
		{
			...
			lr0<-rep(1,M)
			print_if_verbose('========= Performance plot ============')
...
# write.table('-----------------------------------------',file=filen,append=TRUE,row.names=FALSE,col.names=FALSE)
  ...
  write.table('\n',file=filen,append=TRUE,row.names=FALSE,col.names=FALSE,quote=FALSE)
			...
			Dplot<-function()
			{
				...
				print_if_verbose('-- Determination of PrD ranges in progress --')
				...
				ranges0<-range(r0,r1)
				print_if_verbose('======== Sensitivity analysis ===========')
				xp<-as.numeric(tclvalue(ncHp))
				xd<-as.numeric(tclvalue(ncHd))
				theta0<-as.numeric(tclvalue(theta))
